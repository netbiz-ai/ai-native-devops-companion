...
# True when this route's identity chain holds only within its declared limit:
# the digest delivery promoted is not the digest the workload runs, and the
# manifest says why. verify_identity_chain has already refused the case where
# that gap exists and the manifest does not declare it.
identity_limited() {
  local promoted workload
  promoted="$(evidence_field evidence/capstone/delivery/gates.txt allowed_digest)"
  workload="$(sed -n 's/.*workload_image=[^@]*@\([^ ]*\).*/\1/p' "$identity_evidence" | head -1)"
  [[ -n "$promoted" && -n "$workload" && "$promoted" != "$workload" ]]
}

criteria_summary() {
  # The summary is read by people skimming and by anything grepping the output,
  # and neither should have to open the manifest to learn that this route never
  # ties the promoted digest to the running one. An unqualified "consistent"
  # here was the same defect CAP-03 had: a conclusion printed rather than
  # derived.
  if identity_limited; then
    printf 'release identity: consistent within stated limit'
    printf ' (promoted digest is not tied to the workload on this route)\n'
    printf 'release_identity=consistent-within-limit limit=promoted-to-workload\n'
  else
    printf 'release identity: consistent\n'
    printf 'release_identity=consistent\n'
  fi
...
case "$mode" in
  ...
  final)
    ...
    if identity_limited; then
      printf 'CAPSTONE RESULT: PASS WITHIN STATED LIMIT\n'
      printf 'CAP-01 through CAP-06: pass, CAP-01 within its declared limit\n'
    else
    ...
    if identity_limited; then
      printf 'capstone_final=pass-within-limit limit=promoted-to-workload '
      printf 'criteria=CAP-01..CAP-07 cleanup=recorded run_id=%s\n' "$active_run"
    else
