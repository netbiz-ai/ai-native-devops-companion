...
if [ -f "$doc" ]; then
  ...
  if grep -q "SYSTEM OVERRIDE" "$doc"; then
    check "no Break It poison left behind" 1 "run labs/ch14/04-restore-knowledge.sh"
  else
  ...
  check "knowledge doc exists ($doc)" 1 "create it in Step 1 from labs/ch14/config/01-knowledge-doc-frontmatter.md"
fi
