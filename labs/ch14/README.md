...
No shipped script reads that record directly, so the labs run without it, but the knowledge doc you write from it is the chapter's actual work - the shipped test suite passes on an untouched clone and does not evidence that work.
