...
usage() {
  cat <<'USAGE'
Usage: labs/observability/validate.sh [--service-url URL] [--evidence PATH] [--namespace NS]

  --service-url  Probe this URL directly instead of exec'ing into the
                 in-cluster client. Use it with the port-forward from
                 labs/observability/06-port-forward-service.sh.
...
# labs/observability/06-port-forward-service.sh sets up.
if [ -z "$service_url" ] && ! kubectl -n "$namespace" get pod reference-client >/dev/null 2>&1; then
  ...
  printf '        or probe from outside: labs/observability/validate.sh --service-url http://127.0.0.1:8080\n'
