...
- **07 names a test that does not exist.** `tests.test_app.RouteTests.test_health_returns_200` is absent; the shipped test is `RouteTests.test_health_contract`. Chapter 3 prints a third name for the same test, `test_health_is_live`, so one test carries three names across the book and the tree; `labs/reference-app/README.md` maps the Chapter 3 names.
