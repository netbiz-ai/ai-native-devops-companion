...
These three things are named by subject, not by chapter, so reordering the book cannot invalidate them.
`labs/cluster/` is the reader's path through this folder, one numbered file per block the Bridge to the Cluster interlude prints.
