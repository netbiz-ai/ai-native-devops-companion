...
state="${2:-start}"
# Subject names, not chapter numbers: a chapter number describes where
# work sits in the book rather than what it is, so a renumbering would
# leave the tag resolving silently to the wrong state.
tag="${chapter}-${state}"
