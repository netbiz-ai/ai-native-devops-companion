...
# What the summary claims about identity has to follow what the evidence shows,
# not be printed regardless. The lab route's digests differ by design, so the
# unqualified wording is only correct when they match - which is the case a
# reader on a registry-backed route would actually have.
build_fixture
check "a limited identity chain says so in the summary" pass "consistent-within-limit" all

build_fixture
write_manifest "$RUN" "$WORKLOAD" declared-limit
printf 'criterion=CAP-02 observed_at=%s run_id=%s\nallowed_revision=%s\nallowed_digest=%s\n' \
  "$IDENTITY_AT" "$RUN" "$SOURCE_REV" "$WORKLOAD" \
  >"$fixture/evidence/capstone/delivery/gates.txt"
check "matching digests report an unqualified identity" pass "release_identity=consistent$" all
