...
- `labs/chNN/README.md` lists that chapter's script files in printed order, mapped to the book section each came from, and names any printed block that is not shipped as a file.
...
1. Read `labs/chNN/README.md` and the chapter's entry in `docs/chapter-map.md` first.
...
3. Enter the chapter's starting state.
   Chapters 10 to 13 use `make chNN-start`; every other chapter continues from where the previous chapter left the clone.
4. Run the numbered `labs/chNN/*.sh` files in order, showing the reader each result.
   Copy the chapter's Configuration documents from `labs/chNN/config/` to the destination the chapter README names.
...
- **Do not edit the bodies of `labs/chNN/*.sh` or `labs/chNN/config/*`.**
